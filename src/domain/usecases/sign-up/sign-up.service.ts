...
export async function createAccount(registration: RegistrationEntity, hashedPassword: string, salt: string): Promise<void> {
    try {
        const userId = await repository.users.create({username: registration.username ?? 'test', email: registration.email})
        await repository.accounts.create({account_id: registration.accountId, user_id: userId, password: hashedPassword, permissions: [], salt})
    } catch (e) {
